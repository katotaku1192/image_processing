...
class ColorExtract(object):
    def __init__(self):
        self._red_pub = rospy.Publisher('camera2_red_image', Image, queue_size=1)
        self._detect_pub = rospy.Publisher('camera2_detect_image', Image, queue_size=1)
        self._center_pub = rospy.Publisher('camera2_red_center', UInt16MultiArray, queue_size=1)
        self._image_sub = rospy.Subscriber('/robot2/zed_nodelet/left/image_rect_color', Image, self.callback)
...
if __name__ == '__main__':
    rospy.init_node('camera2_detect_marker')
    color = ColorExtract()
    try:
        rospy.spin()
